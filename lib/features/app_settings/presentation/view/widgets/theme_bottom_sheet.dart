import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';
import 'package:hayat_care/features/app_settings/presentation/view/widgets/selection_option_item.dart';
import 'package:hayat_care/localization/app_localizations.dart';

import '../../cubit/app_settings_cubit.dart';

class ThemeBottomSheet extends StatelessWidget {
  const ThemeBottomSheet({super.key});

  @override
  Widget build(BuildContext context) {
    final isDark = Theme.of(context).brightness == Brightness.dark;

    return Padding(
      padding: EdgeInsets.all(20.w),
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          Container(width: 40.w, height: 4.h, decoration: BoxDecoration(color: Colors.grey[400], borderRadius: BorderRadius.circular(10))),
          SizedBox(height: 20.h),
          Row(
            children: [
              Icon(Icons.wb_sunny_outlined, size: 28.sp),
              SizedBox(width: 10.w),
              Text(AppLocalizations.of(context)!.selectTheme, style: TextStyle(fontSize: 16.sp, fontWeight: FontWeight.w600)),
            ],
          ),
          SizedBox(height: 25.h),
          SelectionOptionItem(
            title: AppLocalizations.of(context)!.lightMode,
            isSelected: !isDark,
            onTap: () {
              if (isDark) context.read<AppSettingCubit>().toggleTheme();
              Navigator.pop(context);
            },
          ),
          SizedBox(height: 10.h),
          SelectionOptionItem(
            title: AppLocalizations.of(context)!.darkMode,
            isSelected: isDark,
            onTap: () {
              if (!isDark) context.read<AppSettingCubit>().toggleTheme();
              Navigator.pop(context);
            },
          ),
          SizedBox(height: 20.h),
        ],
      ),
    );
  }
}